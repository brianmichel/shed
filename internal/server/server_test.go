...
func TestGenerateAgentRunDiffDispatchesDiffCommand(t *testing.T) {
	ctx := context.Background()
	st := store.NewMemoryStore()
	mgr := compute.NewManager(compute.ManagerConfig{DefaultCompute: "exec"})
	if err := mgr.RegisterBuiltin("exec", execCompute{}); err != nil {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/agent-runs/"+run.ID+"/generate-diff", nil)
	req.Header.Set("Authorization", "Bearer api-secret")
	req.SetPathValue("agent_run_id", run.ID)
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	...
	if body.Data.Command != "git -C '/workspace/repo' diff --binary HEAD" {
		t.Fatalf("diff command=%q", body.Data.Command)
	}
	events, _, err := st.ListAgentRunEvents(ctx, run.ID, store.EventListOptions{})
	if err != nil {
		t.Fatal(err)
	...
	found := false
	for _, ev := range events {
		if ev.Type == "repo.diff.generated" {
			found = true
		}
	}
	if !found {
		t.Fatalf("diff event missing: %#v", events)
	}
}
...
func TestPushAgentRunBranchRequiresApprovalAndDispatchesPush(t *testing.T) {
	ctx := context.Background()
	st := store.NewMemoryStore()
	mgr := compute.NewManager(compute.ManagerConfig{DefaultCompute: "exec"})
	if err := mgr.RegisterBuiltin("exec", execCompute{}); err != nil {
	...
	unapprovedReq := httptest.NewRequest(http.MethodPost, "/v1/agent-runs/"+run.ID+"/push-branch", strings.NewReader(`{"approved":false}`))
	unapprovedReq.Header.Set("Authorization", "Bearer api-secret")
	unapprovedReq.SetPathValue("agent_run_id", run.ID)
	unapproved := httptest.NewRecorder()
	srv.ServeHTTP(unapproved, unapprovedReq)
	if unapproved.Code != http.StatusConflict {
		t.Fatalf("unapproved status=%d body=%s", unapproved.Code, unapproved.Body.String())
	}

	req := httptest.NewRequest(http.MethodPost, "/v1/agent-runs/"+run.ID+"/push-branch", strings.NewReader(`{"approved":true}`))
	req.Header.Set("Authorization", "Bearer api-secret")
	req.SetPathValue("agent_run_id", run.ID)
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	...
	if body.Data.Command != "git -C '/workspace/repo' push origin HEAD:"+shellQuote(agentRunWorkBranch(run.ID)) {
		t.Fatalf("push command=%q", body.Data.Command)
	}
	events, _, err := st.ListAgentRunEvents(ctx, run.ID, store.EventListOptions{})
	if err != nil {
		t.Fatal(err)
	...
	found := false
	for _, ev := range events {
		if ev.Type == "repo.push.started" {
			found = true
		}
	}
	if !found {
		t.Fatalf("push event missing: %#v", events)
	}
}
